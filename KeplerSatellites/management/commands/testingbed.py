...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        for norad_id in norad_id_list:
            ...
            tle_query = "https://www.space-track.org/basicspacedata/query/class/tle/NORAD_CAT_ID/"+str(norad_id)+"/metadata/false/distinct/true"

            # Returns data from satcat for norad id, only used for launch data
            satcat_query = "https://www.space-track.org/basicspacedata/query/class/satcat/NORAD_CAT_ID/"+str(norad_id)+"/metadata/false"

            jsonquery = {'identity': settings.USERNAME, 'password': settings.PASSWORD, 'query': satcat_query}
            ...
            try:
                ...
                print "Satellite Added " + str(norad_id)
            except Satellite.DoesNotExist:
                satellite_foreign_key = None
                print Satellite.DoesNotExist

            jsonquery = {'identity': settings.USERNAME, 'password': settings.PASSWORD, 'query': tle_query}
            headers = {'Content-Type': 'application/json', 'Accept': 'text/plain'}
