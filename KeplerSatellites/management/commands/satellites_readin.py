                ...
                for x in satellite_cat:
                    if str(satellite_cat[k].get(u'CURRENT', None)) == 'Y':
                        ...
                        satellite_dict = {
                            ...
                            'site_code': (satellite_cat[k].get(u'SITE', None)),
                            'launch': launch,
                            'site': site_dict[(satellite_cat[k].get(u'SITE', None))],
                            'decay': decay,
                            'rcs_value': (satellite_cat[k].get(u'RCSVALUE', None)),
                            'rcs_size': (satellite_cat[k].get(u'RCS_SIZE', None)),
                            'elset_classification': (satellite_cat[k].get(u'CLASSIFICATION_TYPE', None)),
                            'international_designator': (satellite_cat[k].get(u'INTLDES', None)),
                            'launch_year': (satellite_cat[k].get(u'LAUNCH_YEAR', None)),
                            'launch_num': (satellite_cat[k].get(u'LAUNCH_NUM', None)),
                            'launch_piece': (satellite_cat[k].get(u'LAUNCH_PIECE', None)),
                            'current': (satellite_cat[k].get(u'CURRENT', None)),
                            'comment': (satellite_cat[k].get(u'COMMENT', None)),
                            'originator': (satellite_cat[k].get(u'ORIGINATOR', None)),
                            'failed': False,
